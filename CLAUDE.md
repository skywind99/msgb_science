...
**학생 계정을 만들지 않는다.**
신청은 무계정 + 학년·반·번호·이름 + 6자리 확인코드 방식.
...
- 확인코드는 해시로만 저장한다. 평문은 신청 완료 응답에 한 번만 실린다.
- **확인코드 조회 API에는 반드시 요청 제한을 건다.** 6자리라 무차별 대입이 쉽다.
  이걸 빼먹으면 전교생 명단이 털린다. 조회 기능을 만들 때 같은 커밋에 넣을 것.
