...
## 지금 바로 해야 할 것 (사람만 할 수 있는 일)

코드로 끝낼 수 없어 남아 있는 항목이다. 위에서부터 급하다.

1. **개인정보 보호책임자 정보 채우기** — `client/src/pages/Privacy.tsx` 의 `CONTACT`.
   이름·직위·연락처. 비어 있으면 방침 페이지 상단에 경고 상자가 뜬다.
   **학생 정보를 이미 받고 있으므로 이게 가장 급하다.**
2. **Vercel 함수 리전을 서울로** — 대시보드 Settings → Functions → Function Region →
   Seoul (icn1) → 재배포. 지금은 저장은 서울인데 처리가 미국을 거친다.
   바꾼 뒤 방침 5번을 "국외 이전 없음" 으로 고칠 것.
3. **keep-alive 첫 실행 확인** — Actions 탭에서 `Keep Everything Alive` 수동 실행.
   노란 경고 없이 "지울 신청 기록이 없다" 가 나오면 정상.
4. 학교 개인정보 보호책임자와 사전 협의 (아래 개인정보·운영 참고)
...
- **교사만 로그인한다.** Supabase Auth 사용, 자율 가입 없이 초대 링크로만 계정 생성.
  로그인은 이메일이 아니라 아이디다 (2026-07-28). `shared/teacherId.ts` 참고.
  메일을 못 받으므로 관리자용 비밀번호 재설정이 유일한 복구 수단이다.
- **개인정보는 최소한만, 짧게.** 연락처·이메일·생년월일 수집 안 함 (교사도 이메일을 받지 않는다).
  활동 종료 30일 후 신청 기록 자동 삭제.
...
- [x] **상단 메뉴 줄바꿈 수정** (2026-07-28) — 관리자로 로그인하면 버튼 4개가 늘어
      라벨이 단어 중간에서 잘렸다 ("과학실 소 / 개"). 고친 방법:
      헤더 컨테이너만 `max-w-7xl` → `max-w-[110rem]`, 라벨에 `whitespace-nowrap`,
      항목 여백·글자 축소(`px-4`→`px-3`, `text-sm`→`text-[13px]`),
      데스크톱 메뉴 기준을 `lg`(1024px) → `xl`(1280px) 로 올려 좁은 화면은 햄버거로.
- [ ] 메뉴 항목이 8개라 여전히 빡빡하다. 카테고리 5개를 드롭다운 하나로 묶으면
      여유가 생긴다. 지금은 깨지지 않지만 항목이 더 늘면 다시 문제가 된다.
...
- [x] **활동 하나를 폰 캘린더에 담기** (2026-07-28) — `server/calendar.ts`,
      `GET /api/posts/:id/calendar.ics`. 신청 완료 화면과 활동 상세에 버튼.
      알림은 담긴 뒤 폰이 스스로 띄운다 (하루 전 · 한 시간 전).
- [x] **안드로이드 대응** — 두 번 고쳤다.
      `.ics` 는 크롬이 다운로드해 버리고, 구글 캘린더 웹 주소는 앱이 아니라
      브라우저에서 열린다. 결국 `intent://` 로 시스템의 "일정 추가"를 직접 불러야
      기본 캘린더 앱(구글·삼성)이 열린다. `shared/calendarEvent.ts` 의
      `androidIntentUrl`. 서버 302 로는 안 되고(크롬이 막는다) 클라이언트가
      만들어야 해서 이벤트 생성을 shared 로 옮겼다.
      알림 지정은 여전히 불가 — 캘린더 앱 기본값이 붙는다. 화면에 안내를 띄운다.
      **파일 검증으로는 못 잡히는 문제였다. 기기에서 눌러봐야 알 수 있었다.**
- [x] **아이폰 확인 완료** (2026-07-28) — 캘린더 앱이 바로 열린다.
- [x] **기기 확인 완료** (2026-07-28) — 아이폰·안드로이드 폰·안드로이드 태블릿은
      모두 캘린더 앱으로 열린다. **아이패드·PC 는 파일 다운로드**이고 이는
      브라우저 동작이라 응답 헤더로 바꿀 수 없다 (`inline` 시도 후 되돌림).
      그쪽은 받은 파일을 눌러 열거나 구글 캘린더 링크를 쓰면 된다. 더 파지 않기로 했다.
      TODO 원래 순서(구독 피드 먼저)를 바꿨다 — 학생이 실제로 원하는 것은
      "내가 신청한 활동을 잊지 않기"이고, 구독은 갱신 주기를 우리가 정할 수 없어
      급한 공지에 못 쓴다. 이유는 DEVLOG 참고.
- [ ] `GET /api/calendar.ics` — 활동 일정 전체 구독 피드
  - 한 번 구독하면 새 활동이 자동 추가된다. 같은 `buildCalendar()` 를 쓰면 된다.
  - 카테고리별 피드(`?category=career_program`)도 함께 제공
  - **갱신 주기를 우리가 정할 수 없다.** 구글 캘린더는 몇 시간~하루에 한 번 가져간다.
    급한 공지에는 쓸 수 없다는 것을 안내에 적어야 한다.
- [ ] 구독 방법 안내 페이지 (iOS / Android / PC 캡처 포함)
      이게 없으면 구독 피드는 주소만 던지는 셈이 된다. 피드와 같이 낼 것.
...
- [x] keep-alive 워크플로 손봄 (2026-07-28) — 주기 2일, 단계 순서 정리(핑을 맨 뒤로),
      빈 커밋에 `[skip ci]` 추가, 시크릿 누락 시 경고.
      **빈 커밋은 지우면 안 된다** — GitHub 이 60일 무활동 시 예약 워크플로를 끄는 것을
      막는 장치다. 재배포만 없앴다.
