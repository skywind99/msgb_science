...
- [x] **안드로이드 대응** — 크롬이 `.ics` 를 다운로드해 버려서 캘린더가 열리지 않았다.
      구글 캘린더 TEMPLATE 주소(`/api/posts/:id/calendar/google`, 302)를 추가하고
      기기에 맞는 쪽을 먼저 보여준다. 구글 경로는 알림 지정이 불가해
      기본 설정(보통 30분 전)이 적용된다 — 화면에 안내를 띄운다.
      **파일 검증으로는 못 잡히는 문제였다. 기기에서 눌러봐야 알 수 있었다.**
- [x] **아이폰 확인 완료** (2026-07-28) — 캘린더 앱이 바로 열린다.
- [ ] **안드로이드 재확인** — 구글 캘린더 링크로 고친 뒤 아직 안 눌러봤다.
